...
async def _read_events(callback, inst):
    global DEVMON_URI    
    async with websockets.connect(DEVMON_URI) as websocket:        
        async for message in websocket:
            keepReading = callback(message, inst)
            if not keepReading:
                return

def read_events(callback, inst):
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)

    asyncio.get_event_loop().run_until_complete(_read_events(callback, inst))